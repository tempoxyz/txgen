...
| `--reorg-gap <BLOCKS>` | After resolving each synthetic side chain with `DEPTH` canonical blocks, add this many more canonical blocks before starting the next one. Defaults to `0`; requires `--reorg` |
...
For `send-blocks`, aggregate run rates use benchmark wall-clock duration. Per-block timestamps remain the original chain timestamps from the input. With `--reorg`, canonical block stats remain canonical-only, but the wall-clock duration includes synthetic fork block build/submission work. Each cycle submits `DEPTH` synthetic blocks, resolves them with the corresponding `DEPTH` canonical blocks, then submits the additional canonical blocks requested by `--reorg-gap`. The gap defaults to `0`, so synthetic side chains never overlap.
